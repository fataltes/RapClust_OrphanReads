...
def processQuant(config):
    ...
    eqnet.buildNetFile(sampleDirs, netFile, orphanFile, cutoff, auxDir)

    # Filter the network
    filtNetFile = os.path.sep.join([outdir, "mag.filt.net"])
    infoFiltNetFile = os.path.sep.join([outdir, "mag.filt.txt"])
    ...
    #call(["Infomap/Infomap", infoFiltNetFile, outdir,
    #     "--clu", "--undirected", "--zero-based-numbering", "--hard-partitions", "--out-name", "mag.flat.clust"])
    #infomapClusFile = os.path.sep.join([outdir, "mag.flat.clust.clu"])
    #eqnet.convert2clusterFormat(infomapClusFile, clustFile, flatClustFile)
